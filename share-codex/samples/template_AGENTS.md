...
- 進め方:
  - まず質問で回収し（テンプレ本文中の `TODO:` を埋めるための情報を回収する）、聞き忘れがないか点検した上で、本文を書き始めてよいか合意を取ってから本文を書く（合意がない限り本文を書かない）。合意後は、書き込み前に矛盾や致命的な抜けがないかだけ見直し、問題があればその点だけ質問してから本文を書く。
  - 質問の順番は固定しない。相手が答えやすい流れに並べる。「目的」など長くなりやすいテーマは段階的に聞く（短い要約→補足質問）。
- 仮置き:
...
## プロジェクトの目的
TODO: 以下の「AGENTS.md作成時の注意点」をもとに、リポジトリの目的を記録する（この章は、このリポジトリで「何を作るか」「何を作らないか」を短く明文化し、判断基準にする）。実施: ユーザーに「何を達成したいか」を確認し、2〜5行で目的文としてまとめる。目的文で曖昧な語やこのリポジトリ固有の語を使う場合は、その意味を確認して用語として補足する。
...
TODO: 以下のディレクトリの説明を完成させる（各行は「パス: 短い呼び名 + 役割/中身 + 必要なら選択肢」の順で書く）。実施: （選択肢: ...）が付いている項目は、採用するパスをユーザーに確認して1つに確定する（例: `_notes/` と `notes/` のどちらを使うかを確定する）。確定後は未採用の選択肢や矛盾する記述を削除する。

ベースライン（TODO: 本リポジトリで採用するディレクトリだけに絞る。採用しないものは削除し、（選択肢: ...）が残る場合は採用案を確定して選択肢表記を消す）:
...
- `docs/web/`: 外部参照ログ。外部情報の参照記録。コミット可否と取り扱いルールを明記する。（選択肢: `_data/web/`）
  - 置き方の規則: `docs/web/<サイト名（短）>/<タイトル>.md`（または `_data/web/<サイト名（短）>/<タイトル>.md`）
  - 注意: `<タイトル>` に日付は付けない
